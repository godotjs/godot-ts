/**
 * Expose as a class.
 * An JavaScript object is created and attached automaticly when construct an instance from this class
 */
...
function make_rpc_mode_config_decorator(mode: godot.MultiplayerAPI.RPCMode) {
  return function <T extends godot.Node>(
    target: T,
    property: string,
    descriptor?: any,
  ) {
 ...
 */
export const remote = make_rpc_mode_config_decorator(
  godot.MultiplayerAPI.RPC_MODE_REMOTE,
);
/**
 * Used with `Node.rpc_config` or `Node.rset_config` to set a method to be called or
 ...
 */
export const master = make_rpc_mode_config_decorator(
  godot.MultiplayerAPI.RPC_MODE_MASTER,
);
/**
 * Used with `Node.rpc_config` or `Node.rset_config` to set a method to be called or
 ...
 */
export const puppet = make_rpc_mode_config_decorator(
  godot.MultiplayerAPI.RPC_MODE_PUPPET,
);
/**
 * @deprecated Use `RPC_MODE_PUPPET` (@puppet) instead. Analogous to the `slave` keyword.
 */
export const slave = make_rpc_mode_config_decorator(
  godot.MultiplayerAPI.RPC_MODE_SLAVE,
);
/**
 * Behave like `RPC_MODE_REMOTE` (@remote) but also make the call or property change locally.
 * Analogous to the `remotesync` keyword.
 */
export const remote_sync = make_rpc_mode_config_decorator(
 ...
 */
export const master_sync = make_rpc_mode_config_decorator(
  godot.MultiplayerAPI.RPC_MODE_MASTERSYNC,
);
/**
 * Behave like `RPC_MODE_PUPPET` (@puppet) but also make the call or property change locally.
 * Analogous to the `puppetsync` keyword.
 */
export const puppet_sync = make_rpc_mode_config_decorator(
  godot.MultiplayerAPI.RPC_MODE_PUPPETSYNC,
);
